...
%{Intro%} << { \mylv gui2\laissezVibrer\f\fermata } \\ { gui4 r } >> | r1\fermata
    ...
    { | << { \mylv gui4\laissezVibrer\f } \\ { gui4 } >> r4 | R2*7
      | \mark #24 << { \mylv gui4\laissezVibrer } \\ { gui4 } >> r4 | R2*7
      | \mark #25 \time 4/4 << {\mylv gui2\laissezVibrer\f} \\ {gui4 r} >> r2
      ...
      | R1*3 | \mark #26 << {\mylv gui2\laissezVibrer\f} \\ {gui4 r} >> r2 % in this bar in the original, there is an example similar to lilyponds poor automatic formatting
      | r1 | << { \mylv gui2\laissezVibrer} \\ { gui4 r} >> r2 | r1
    }
    \alternative
  { { | \mark #27 % this is one of the rare cases where the first measure of D is different between the repeats
R1*2 }
